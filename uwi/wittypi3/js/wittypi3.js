var missingWittyPi3 = function() {
  var dlg = $('#MissingWittyPi3');
  if (dlg.length == 0 || !dlg.dialog('isOpen')) {
    dlg.remove();
    dlg = $("<div id='MissingWittyPi3' title='Witty Pi 3 not found'><p>Witty Pi 3 uses I2C address 0x68 and 0x69. However these two addresses are not listed when running \"i2cdetect -y 1\".</p><p>Please check the physical connection between Witty Pi 3 and Raspberry Pi.</p></div>");
    dlg.dialog({
      autoOpen: true,
      closeOnEscape: false,
      height: 160,
...
var missingSoftware = function() {
  ...
  if (dlg.length == 0 || !dlg.dialog('isOpen')) {
    dlg.remove();
    dlg = $("<div id='MissingSoftware' title='Witty Pi 3 software missing'><p>Witty Pi 3's software should be installed beforehand.</p><p>Usually it is installed to the \"/home/pi/wittypi\" directory. If it was installed to a different location, please configure accordingly in the \"uwi.conf\" file.</p></div>");
    dlg.dialog({
      autoOpen: true,
      closeOnEscape: false,
      height: 160,
...
var refreshOverview = function() {
  ...
  sharedWS.addCompositeTask([
    { msg: 'wittypi3|api_get_temperature', callback: function(event) {
      $('#temperature').text(event.data);
    }},
    { msg: 'wittypi3|api_get_sys_time', callback: function(event) {
      $('#rpi_time').text(event.data);
    }},
    { msg: 'wittypi3|api_get_rtc_time', callback: function(event) {
      $('#rtc_time').text(event.data);
    }},
    { msg: 'wittypi3|api_get_power_mode', callback: function(event) {
      if (event.data == 1) {
        vin.show();
      } else {
        vin.hide();
    ...
    { msg: 'wittypi3|api_get_input_voltage', callback: function(event) {
      if (vin.is(":visible")) {
        vin.text('Vin=' + Number(event.data).toFixed(2) + 'V, ');
      }
    }},
    { msg: 'wittypi3|api_get_output_voltage', callback: function(event) {
      $('#v_out').text('Vout=' + Number(event.data).toFixed(2) + 'V, ');
    }},
    { msg: 'wittypi3|api_get_output_current', callback: function(event) {
      $('#i_out').text('Iout=' + Number(event.data).toFixed(2) + 'A');
    }},
    { msg: 'wittypi3|api_script_inuse', callback: function(event) {
      $('#schedule_script').html(event.data == 'no' ? "&nbsp;" : " (in use) ");
    }}
  ], true);
};
...
var refreshSchedule = function() {
  sharedWS.addCompositeTask([
    { msg: 'wittypi3|api_get_shutdown_time', callback: function(event) {
      $('#shutdown_time').html(format_date_time(event.data, "&nbsp;"));
    }},
    { msg: 'wittypi3|api_get_startup_time', callback: function(event) {
      $('#startup_time').html(format_date_time(event.data, "&nbsp;"));
    }},
    { msg: 'wittypi3|api_script_inuse', callback: function(event) {
      $('#schedule_script').html(event.data == 'no' ? "&nbsp;" : " (in use) ");
    }}
...
var refreshSettings = function() {
	sharedWS.addCompositeTask([
    { msg: 'wittypi3|api_get_low_voltage', callback: function(event) {
      $('#low_voltage').text(event.data);
    }},
    { msg: 'wittypi3|api_get_recovery_voltage', callback: function(event) {
      $('#recovery_voltage').text(event.data);
    }},
    { msg: 'wittypi3|api_get_default_state', callback: function(event) {
    	$('#default_state').text(event.data);
    }},
    { msg: 'wittypi3|api_get_cut_power_delay', callback: function(event) {
      $('#cut_power_delay').text(event.data);
    }},
    { msg: 'wittypi3|api_get_pulsing_interval', callback: function(event) {
      $('#pulsing_interval').text(event.data);
    }},
    { msg: 'wittypi3|api_get_led_duration', callback: function(event) {
      $('#led_duration').text(event.data);
    }},
    { msg: 'wittypi3|api_get_dummy_load_duration', callback: function(event) {
      $('#dummy_load_duration').text(event.data);
    }},
    { msg: 'wittypi3|api_get_vin_adjustment', callback: function(event) {
      $('#vin_adjustment').text(event.data);
    }},
    { msg: 'wittypi3|api_get_vout_adjustment', callback: function(event) {
      $('#vout_adjustment').text(event.data);
    }},
    { msg: 'wittypi3|api_get_iout_adjustment', callback: function(event) {
...
var sys_to_rtc = function() {
  sharedWS.addTask('wittypi3|api_system_to_rtc', function(event) {
    refreshOverview();
  });
};

var rtc_to_sys = function() {
  sharedWS.addTask('wittypi3|api_rtc_to_system', function(event) {
    refreshOverview();
  });
};

var sync = function() {
  sharedWS.addTask('wittypi3|api_sync_time', function(event) {
    refreshOverview();
  });
};
...
var save_shutdown = function(dt) {
  if (dt.length > 0) {
    ...
    day = dt[0].split('-')[2];
    t = dt[1].split(':');
    sharedWS.addTask('wittypi3|api_set_shutdown_time|' + day + '|' + t[0] + '|' + t[1] + '|' + t[2], function(event) {
      refreshSchedule();
    });
  } else {
    sharedWS.addTask('wittypi3|api_clear_shutdown_time', function(event) {
      refreshSchedule();
    });
  }
};
...
var save_startup = function(dt) {
  if (dt.length > 0) {
    ...
    day = dt[0].split('-')[2];
    t = dt[1].split(':');
    sharedWS.addTask('wittypi3|api_set_startup_time|' + day + '|' + t[0] + '|' + t[1] + '|' + t[2], function(event) {
      refreshSchedule();
    });
  } else {
    sharedWS.addTask('wittypi3|api_clear_startup_time', function(event) {
      refreshSchedule();
    });
  }
};
...
var list_scripts = function(d) {
  ...
  btn.button('disable');
  sharedWS.addTask('wittypi3|api_list_scripts', function(event) {
    var code = '';
    ...
    $('#scriptMenu').menu({
      select: function(ev, ui){
        sharedWS.addTask('wittypi3|api_load_script|' + ui.item[0].innerText, function(event) {
...
var set_script = function() {
  ...
  dlg.dialog({
      ...
      buttons: [
        ...
        { text:"OK", click:function(){
            sharedWS.addTask('wittypi3|api_apply_script|' + btoa($('#script').val()), function(event) {
              refreshSchedule();
            });
            $(this).dialog("close");
          }
  ...
  sharedWS.addTask('wittypi3|api_current_script', function(event) {
    $('#script').val(atob(event.data));
  });
};
...
var set_low_voltage = function() {
  ...
  inputValue('Low Voltage Threshold',
	  ...
	  function(val) {
        ...
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_low_voltage|' + parseInt(val*10), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_low_voltage', callback: function(event) {
	       $('#low_voltage').text(event.data);
	      }}
...
var set_recovery_voltage = function() {
  ...
  inputValue('Recovery Voltage Threshold',
	  ...
	  function(val) {
	  	if (val != null && val >= 0 && val <= 25) {
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_recovery_voltage|' + parseInt(val*10), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_recovery_voltage', callback: function(event) {
	       $('#recovery_voltage').text(event.data);
	      }}
...
var set_default_state = function() {
	chooseValue('Default State', 
	  ...
	  function(val) {
	  	sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_default_state|' + val, callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_default_state', callback: function(event) {
		      $('#default_state').text(event.data);
		    }}
...
var set_pulsing_interval = function() {
  var labels = ['1 Second', '2 Seconds', '4 Seconds', '8 Seconds'];
	...
	chooseValue('Pulsing Interval',
	  ...
	  function(val) {
	  	sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_pulsing_interval|' + val, callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_pulsing_interval', callback: function(event) {
		      $('#pulsing_interval').text(event.data);
		    }}
...
var set_led_duration = function() {
	inputValue('White LED Duration',
	  ...
	  function(val) {
        ...
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_led_duration|' + parseInt(val), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_led_duration', callback: function(event) {
	       $('#led_duration').text(event.data);
	      }}
...
var set_dummy_load_duration = function() {
	inputValue('Dummy Load Duration',
	  'Please input the duration for applying dummy load, when the pulse comes:',
	  ...
	  function(val) {
	  	if (val != null && val >= 0 && val <= 255) {
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_dummy_load_duration|' + parseInt(val), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_dummy_load_duration', callback: function(event) {
	       $('#dummy_load_duration').text(event.data);
	      }}
...
var set_vin_adjustment = function() {
	inputValue('Vin Adjustment',
	  ...
	  function(val) {
	  	if (val != null && val >= 0 && val <= 255) {
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_vin_adjustment|' + parseInt(val*100), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_vin_adjustment', callback: function(event) {
	        $('#vin_adjustment').text(event.data);
	      }},
...
var set_vout_adjustment = function() {
	inputValue('Vout Adjustment',
	  ...
	  function(val) {
	  	if (val != null && val >= 0 && val <= 255) {
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_vout_adjustment|' + parseInt(val*100), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_vout_adjustment', callback: function(event) {
	        $('#vout_adjustment').text(event.data);
	      }},
...
var set_iout_adjustment = function() {
	inputValue('Iout Adjustment',
	  ...
	  function(val) {
	  	if (val != null && val >= 0 && val <= 255) {
        sharedWS.addCompositeTask([
		    { msg: 'wittypi3|api_set_iout_adjustment|' + parseInt(val*100), callback: function(event) {
		    }},
		    { msg: 'wittypi3|api_get_iout_adjustment', callback: function(event) {
	        $('#iout_adjustment').text(event.data);
	      }},
